...
export default function Dashboard() {
  ...
  // Number of stakeholders without an organization
  ...
  useEffect(() => {
    ...
    // Check for stakeholders that do not have an organization
    contactsApi
      .unassigned()
      .then((res) => {
        setUnassignedCount(res.count || 0);
      })
      .catch(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Unassigned stakeholder warning */}
      {unassignedCount > 0 && (
        <Card className="border-amber-200 bg-amber-50 p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-amber-900">
                  Stakeholders need organization mapping
                </p>

                <p className="mt-1 text-sm text-amber-700">
                  {unassignedCount} stakeholder
                  {unassignedCount !== 1 ? "s" : ""} currently{" "}
                  {unassignedCount !== 1 ? "have" : "has"} no organization.
                </p>
              </div>
            </div>
            ...
            <Link
              to="/contacts"
              className="inline-flex h-10 items-center justify-center rounded-xl bg-amber-600 px-4 text-sm font-semibold text-white transition hover:bg-amber-700"
            >
              Review Contacts
            </Link>
          </div>
...
function EngagementChart({ trend }) {
  ...
  const prev =
    maxIndex > 0 ? counts[maxIndex - 1] : 0;

  const growth =
    prev > 0
      ? Math.round(((max - prev) / prev) * 1000) / 10
      : 17.8;
